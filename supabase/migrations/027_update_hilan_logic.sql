-- Upgrade create_employee_event to support full Hilan Action Code Logic
-- Fixes:
-- 1. Op ' ' (Rishum): BLOCKS if future record exists.
-- 2. Op '4' (Achifa) / '2' (Tiuv): Caps new record at next record's start date (Historical Insert).
-- 3. Ensures no overlapping validity periods.
...
BEGIN
    ...
    IF p_event_code = '100' THEN
        ...
        IF p_operation_code = ' ' THEN
            ...
            IF FOUND THEN
                -- Exact match -> Correction (allowed per Hilan logic simplified, or should we block 'Add' on existing? 
                -- Hilan says: "When event exists... update by Replacement ('2')".
                -- But usually 'Add' on existing key allows update in permissive systems. 
                -- STRICT MODE: Block? 
                -- Let's stick to "Block Future" for ' '.
                -- Exact match is NOT future, so we can allow Update.
            ...
            ELSE
                ...
                IF EXISTS (SELECT 1 FROM employee_name_history WHERE employee_id = v_employee_id AND effective_from > v_effective_from) THEN
                    RETURN json_build_object('success', false, 'error', 'Cannot use Rishum (Add) when a later event exists. Use Achifa (Set).');
                END IF;
