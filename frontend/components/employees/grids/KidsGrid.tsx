    ...
    const handleAddNew = async () => {
        try {
            ...
            await dispatchEmployeeAction(organizationId, employeeId, {
                ...
                request_id: `family-add-${employeeId}-${Date.now()}`,
                payload: {
                    first_name: '',
                    last_name: '',
                    gender: 'M',
            ...
            <table className="w-full text-xs text-right border-collapse">
                <thead className="bg-[#EAECEE] text-gray-700 font-bold sticky top-0 z-10">
                    <tr>
                        ...
                        <th className="p-1 px-2 border-l border-b border-white last:border-l-0 w-[110px]">מתאריך</th>
                        <th className="p-1 px-2 border-l border-b border-white last:border-l-0 w-[110px]">עד תאריך</th>
                        <th className="p-1 px-2 border-b border-white w-[50px] text-center">פעולות</th>
                    </tr>
                </thead>
                <tbody>
                    {loading ? (
                        <tr><td colSpan={8} className="p-4 text-center text-gray-400">טוען נתונים...</td></tr>
                    ) : (
                        <>
                            {data.map((row) => (
                            ...
                            >
                                <td colSpan={8} className="p-2 text-center text-gray-500 font-medium text-xs border-l-4 border-l-transparent group-hover:border-l-primary flex items-center justify-center gap-2">
