...
export default function EditSubscriptionTierPage() {
  ...
  const fetchTier = async () => {
    try {
      ...
      setFormData({
        ...
        included_modules: data.included_modules || ['core'],
      ...
      toast.error('שגיאה בטעינת סוג המנוי')
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('סוג המנוי עודכן בהצלחה!')
      router.push('/admin/subscription-tiers')
    } catch (err: any) {
      console.error('Error updating subscription tier:', err)
      const errorMessage = err.message || 'שגיאה בעדכון סוג המנוי'
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-text-primary">עריכת סוג מנוי</h1>
        <p className="text-text-secondary mt-2">עדכן את פרטי סוג המנוי</p>
      </div>
