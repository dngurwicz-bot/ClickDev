"""
Script to patch the EmployeeDetails.tsx file.
Removes specific buttons and updates title styles.
"""

FILE_PATH = (
    r"c:\ClickDev\ClickDev\frontend\components\employees\EmployeeDetails.tsx"
)
...
# Making it standard black/gray
NEW_TITLE_STYLE = 'text-gray-900 font-bold'
...
# We want to keep the "More Horizontal" button but remove the "Salary"
# and "Building" buttons.
# The buttons are:
...
# There are multiple of these. The one in ACTION TOOLBAR is inside the div
# with `{/* 2. ACTION TOOLBAR ... */}`
...
if start_toolbar_idx != -1:
    ...
    toolbar_inner_start = content.find(
        CONTAINER_START_MARKER, start_toolbar_idx
    )
    if toolbar_inner_start != -1:
        # The content to REPLACE is everything inside this div UNTIL the
        # "MoreHorizontal" button.
        # But wait, looking at the file (lines 575-591 in previous view),
        # the buttons are INSIDE this div.
        ...
        REPLACEMENT_INNER_HTML = (
            ...
            '<Button variant="ghost" className="h-7 px-2 text-gray-600 '
            'hover:bg-gray-100 gap-1 text-xs font-normal rounded-sm">\n'
            '   <MoreHorizontal className="w-3.5 h-3.5" />\n'
            '   <span>פעולות</span>\n'
            '</Button>'
        )

        # We need to find where the "MoreHorizontal" button starts in the
        # CURRENT content to verify we are replacing safely?
        # OR just identifying the start and end of the block to remove.

        # The block to remove starts after `gap-2">` and ends before
        # `<Button variant="ghost" ...> <MoreHorizontal`.
        # Actually it also includes the divider.

        # Let's try replacing the entire block of buttons if we can match it
        # string-wise. It's risky with whitespace.

        # Strategy: matching the known unwanted strings and replacing them
        # with empty strings?
        ...
        if SALARY_BTN_SNIPPET in content and COMPANY_BTN_SNIPPET in content:
            ...
            # From `start_toolbar_idx` to `div className="flex items-center
            # gap-2"` (right side actions)

            # Find the END of the left-side toolbar div.
            # It ends before `<div className="flex items-center gap-2">`
            # (the printer icons).
            ...
            # I'll re-read the file line by line and filter out lines
            # containing specific unique keywords if they are within the
            # expected range.
            ...
            # Keywords to trigger skip: '<span>נתוני שכר לעובד</span>' ? No,
            # that's inside the button.
            ...
            for i, line in enumerate(lines):
                ...
                if '<span>נתוני שכר לעובד</span>' in line:
                    # We are INSIDE the first button. We need to have started
                    # skipping BEFORE this.
                    pass

                # Logic: Build a new list of lines.
                # If a line contains specific unique unwanted content, don't
                # add it?
                # The button is multiline.

                # Let's do a replace on the huge chunk if possible.

            # Let's try the replace string method which is safer if I can
            # construct the string from the previous `view_file` output
            # exactly.
            # I can copy paste the previous view output into this script as
            # the "target".

            TARGET_CHUNK = (
                '<Button variant="ghost" className="h-7 px-2 text-[#1f497d] '
                'hover:bg-[#e3f2fd] gap-1.5 text-xs font-bold rounded-sm '
                'border border-transparent hover:border-[#bbdefb]">\n'
                ...
                if div_loc != -1:
                    ...
                    if btn1_start < btn2_start < div_loc:
                        ...
                        content = content.replace(
                            to_remove, "{/* Removed irrelevant actions */}"
                        )
